...
type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, "FieldChecklist">;
  route: RouteProp<RootStackParamList, "FieldChecklist">;
};
...
function ClientSelector({ clients, value, onChange, onCreated }: {
...
export default function FieldChecklistScreen({ navigation, route }: Props) {
  const insets = useSafeAreaInsets();
  const { inspectorId, userName, type, typeLabel, typeIcon } = route.params;
  ...
  const loadChecklist = async () => {
    ...
    try {
      const r = await axios.get<{ categories: CheckCategory[] }>(`${API_URL}/mobile/checklist?type=${encodeURIComponent(type)}`);
      const cats = r.data.categories ?? [];
      setCategories(cats);
      const items: CheckItem[] = [];
      for (const cat of cats) {
        for (const item of cat.items) items.push({ category: cat.label, item, result: "ok" });
      }
      setCheckItems(items);
      setStep("checklist");
    } catch {
      Alert.alert("Error", "No se pudo cargar la lista de verificación.");
    } finally {
  ...
  const uploadPhoto = async (uri: string, inspId: string) => {
    ...
    form.append("entity_type", "field_inspection");
    form.append("entity_id", inspId);
  ...
  const submit = async () => {
    ...
    try {
      ...
      const { data } = await axios.post<{ ok: boolean; id: string; folio: string }>(`${API_URL}/mobile/field-inspection`, {
        type, client_id: clientId || undefined, location, service_date: serviceDate,
        technician, result, observations, checklist: checkItems,
      });
      for (const uri of photos) await uploadPhoto(uri, data.id).catch(() => {});
      Alert.alert(
        "✅ Inspección guardada",
        `Folio: ${data.folio}\n${result === "ok" ? "Sin fallas críticas" : result === "requires_attention" ? "Requiere atención" : "Fuera de servicio"}`,
    ...
    return (
      <KeyboardAvoidingView style={{ flex:1, backgroundColor:"#F4F7FB" }} behavior={Platform.OS==="ios"?"padding":undefined}>
        <View style={s.nav}>
          ...
          <Text style={s.navTitle}>{typeIcon} {typeLabel}</Text>
          <View style={{ width:70 }} />
        </View>
        <ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
          <View style={s.card}>
            <Text style={s.cardTitle}>Datos de la inspección</Text>
      ...
      <ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
        <View style={[s.card, { backgroundColor:result==="ok"?"#D1FAE5":result==="critical"?"#FEE2E2":"#FEF3C7", borderWidth:1.5, borderColor:result==="ok"?"#10B981":result==="critical"?"#EF4444":"#F59E0B" }]}>
          <Text style={{ fontSize:18, fontWeight:"900", color:result==="ok"?"#065F46":result==="critical"?"#991B1B":"#92400E", textAlign:"center" }}>
            {result==="ok"?"✓ Aprobado":result==="critical"?"✗ No aprobado":"⚠ Requiere atención"}
          </Text>
        ...
        <TouchableOpacity onPress={submit} disabled={saving} style={[s.btnPrimary, { marginHorizontal:0, opacity:saving?0.6:1 }]}>
          {saving ? <ActivityIndicator color="#fff" /> : <Text style={s.btnPrimaryTxt}>Guardar inspección</Text>}
        </TouchableOpacity>
      </ScrollView>
    </KeyboardAvoidingView>
  );
...
function styles(insets: { top: number; bottom: number }) {
  return {
    ...
    navTitle: { color:"#fff", fontWeight:"700" as const, fontSize:15, flex:1, textAlign:"center" as const },
